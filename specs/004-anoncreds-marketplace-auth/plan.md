...
| R8 | GraphDB sync for new registries | ✅ **Landed — Vote/GrantProposal/Pledge/MatchInitiation mirrored to GraphDB** | `apps/web/src/lib/ontology/graphdb-sync.ts` gained `emitSpec004RegistriesTurtle` + a config-driven `emitOneSpec004Registry` that walks each registry's `allSubjects()`, reads predicates via the generic `getBytes32`/`getString`/`getUint`/`getAddress` getters, and emits Turtle keyed by `urn:smart-agent:<class>:<subjectHex>`. The discriminator predicate (ballot/round/pool/viewedIntent) keeps non-class attribute-storage subjects from being miscounted. The orchestrator (`syncOnChainToGraphDB`) appends the four classes to the combined data-graph upload. Smoke-tested against a real vote: `<urn:smart-agent:vote:dfb2…cee4> a sa:Vote ; sa:round 0xba7d… ; sa:proposal 0xaaaa… ; sa:ballot 0x8df6… ; sa:nullifier 0x2c76… ; sa:weight 1 ; sa:castAt … ; sa:updatedAt …` confirmed in GraphDB. Sync message now reports `+ 1 Vote / + N GrantProposal / + N Pledge / + N MatchInitiation` alongside the existing agent / pool / round counts. Story-permissions filter on pledge emit (stripping donor identity when `showName=false`) is queued — the chain doesn't store a donor reveal anyway, so today's emit is already anonymous-by-construction. |
| R10 | FundRegistry voting-config extension + drop `rounds` SQL | ✅ **Landed** | `FundRegistry` gained `setRoundVotingConfig(round, strategy, threshold, windowStartsAt, windowEndsAt)` + `getRoundVotingConfig(round)` view (returns 4-tuple). 4 new attribute predicates (sa:roundVotingStrategy / Threshold / WindowStartsAt / WindowEndsAt) registered via the spec-004 ontology backfill. ABI regenerated. Tool policy for `round:update_voting_config` flipped from `mcpOnly` → `statelessRedeem` (target=FundRegistry, selector=setRoundVotingConfig). org-mcp's `round:get_voting_config` + `vote:tally_for_round` + the web action's `loadRoundConfig` all read from chain via the new getter, with `(steward-quorum, 2, no window)` as the default when unset. Web action's `proposalVotes.action.ts` no longer touches a local SQLite — it goes straight to FundRegistry via the SDK abi. **`rounds` SQL table DROPPED** from `apps/org-mcp/src/db/{schema,index}.ts`; `seed-test-round.ts` updated to no-op the SQL voting-config seed (defaults already match the demo's expectations). |
...
| R9 | SQL table drops | ✅ **Landed — `proposal_submissions` / `proposal_votes` / `pool_pledges` / `match_initiations` removed** | <br/>**org-mcp** — all four tables gone from `db/schema.ts` + `db/index.ts`. Read tools (`vote:list_for_*`, `vote:tally_for_round`, `pool_pledge:read_self`, `pool_pledge:list_for_pool`, `pool_pledge:read_pool_counters`, `grant_proposal:read_self`, `grant_proposal:list_for_member`, `grant_proposal:list_for_round`, `grant_proposal:count_for_round`, `match_initiation:read`) stub to empty until R8 (on-chain → GraphDB sync) ships. Write tools that previously did SQL state transitions (`grant_proposal:draft/clone/award/revoke_award/rescind`) return a "moved on chain" error.<br/>**person-mcp** — `pool_pledges` + `match_initiations` dropped (no draft state for either); tool surfaces stubbed. `proposal_submissions` KEPT for proposer-side draft state (drafts aren't on chain). Seeds rewritten: `seed-test-pool` keeps on-chain pool deploy, `seed-test-round` keeps FundRegistry.openRound, `seed-test-pledge` and `seed-test-match-initiation` are no-ops with explanatory log line, `seed-test-proposal` only writes person-mcp drafts. <br/>**`rounds` table KEPT** — slim voting-config table (votingStrategy / votingThreshold / votingWindowStartsAt / votingWindowEndsAt) until R10 folds those into FundRegistry attributes. <br/>**Smoke-tested post-drop**: voter casts on proposals A and B → two distinct on-chain vote subjects under the same nullifier, ballots correctly stored as `sa:Approve` and `sa:Reject`, cross-round attempt rejected before reaching chain. |
| R9a | grant_proposal poolAgentId hardening | ✅ **Landed** | The verifier's `expectedAttributes.poolAgentId` is now resolved from `FundRegistry.getRoundPoolAgent(roundSubject)` on chain, not from the caller-supplied `args.presentation.poolAgentId`. Previously a malicious caller could pass `poolAgentId = <their-cred's-pool>` and the verifier would happily confirm the cred matches the claim, even if the round being submitted to belonged to a different pool. The new `resolveRoundPoolAgent` helper (`apps/org-mcp/src/tools/grantProposals.ts`) closes that loophole for submit/edit/withdraw. |
