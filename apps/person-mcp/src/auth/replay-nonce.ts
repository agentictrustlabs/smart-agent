/**
 * Replay-nonce cache for the a2a-agent → person-mcp inbound service-auth
 * envelope (Sprint 1 W2.1).
 *
 * Mirrors `apps/a2a-agent/src/auth/replay-nonce.ts`: every authenticated
 * inbound request carries a fresh per-request nonce; the verifier
 * attempts to INSERT it into `inter_service_nonce` and rejects on
 * UNIQUE-constraint collision so a captured envelope can't be replayed
 * within the ±60s timestamp window.
 *
 * Cleanup is wired up in `apps/person-mcp/src/index.ts` at startup
...
export function recordNonce(nonce: string, service: string): boolean {
  ...
  try {
    ...
    console.error('[person-mcp replay-nonce] insert failed:', msg)
    return false
  }
}
