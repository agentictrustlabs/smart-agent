/**
 * Inbound service-auth middleware for person-mcp (Sprint 1 W2.1).
 *
 * Person-mcp's control-plane surfaces (`/session-store/*`, `/audit/append`,
 * `/wallet-action/*`, `/tools/*`) used to accept any caller that could
 * reach the HTTP port. A2A-agent's edge verified the inbound HMAC
 * envelope from web (Stream B Task B1) and from MCPs (K3-ext), but the
 * downstream a2a→person-mcp hop was unauthenticated — if person-mcp's
 * port ever became reachable beyond localhost, a compromised peer or
 * misconfigured proxy could insert sessions, bump revocation epochs,
 * append fake audit entries, etc.
 *
 * This middleware closes the gap. Every authority-bearing route on
 * person-mcp now re-verifies that the caller is an enrolled upstream
 * service (currently only `a2a-agent` is allowed inbound — web shouldn't
 * bypass a2a-agent).
 *
 * Wire format mirrors the web→a2a envelope, since this hop has no
 * session id baked into the path:
 *
 ...
 *
 * MAC key id is `a2a-to-person`. HMAC is symmetric — a2a-agent and
 * person-mcp share the secret (`A2A_INTERSERVICE_HMAC_KEY_PERSON` in
 * local-aes mode, `AWS_KMS_MAC_KEY_ID_A2A_TO_PERSON` in aws-kms mode).
 * a2a-agent already holds it as an OUTBOUND signing key when calling
 * /session/:id/redeem-tx from person-mcp's a2a-client.ts; we now reuse
 * the same key in the OTHER direction so a2a→person can be HMAC-signed
...
/** The only inbound service allowed to talk to person-mcp's control plane. */
...
// Lazy provider cache — the MAC provider for `a2a-to-person` is
...
function inboundMacProvider(): KmsMacProvider {
  if (!cachedMacProvider) {
    // `buildMcpMacProvider('person', env)` returns the provider scoped
    // to the `a2a-to-person` MAC key id. HMAC is symmetric, so the same
  ...
  return async (c: Context, next) => {
    ...
    try {
      ...
      auditDeny(c, {
        ...denyFields,
        mcpServer: service,
        reason: `a2a-to-person key not configured: ${(err as Error).message}`,
      })
      return c.json(
        { error: `a2a-to-person key not configured: ${(err as Error).message}` },
        503,
      )
    }
