/**
 * Tests for the cross-delegation dual-address binding proof on org-mcp
 * (Sprint 4 A.2 — mirrors person-mcp's S2.3 tests).
 *
 * The cross-delegation verifier in
 * `apps/org-mcp/src/auth/verify-delegation.ts::verifyCrossDelegation`
 * previously had a binding gap (the parallel of person-mcp's): it did
 * NOT assert that the caller's session smart-account matched the
 * cross-delegation's `delegate`. A.2 closes the gap via Option C — an
 * in-caveat dual binding:
 *
 ...
 *
 * Run:
 *   node --import tsx --test apps/org-mcp/test/cross-delegation-binding.test.ts
 */
...
// Required by org-mcp config.ts at module load.
...
function buildCrossDelegation(opts: {
  ...
  if (opts.includeDataScope !== false) {
    caveats.push(buildDataScopeCaveat(ORG_GRANTS))
  }
...
test('happy path: caller SA + PA match binding → binding check passes (no binding-level error)', async () => {
  ...
  const result = await verifyCrossDelegation(xdel, BOB_SA, ORG_AUDIENCE)
...
test('reject path: caller SA does NOT match binding.delegateSmartAccount → binding mismatch error', async () => {
...
test('reject path: legacy cross-delegation (no DelegateBinding caveat) → rejected unless dev compat flag is set', async () => {
  ...
  const r1 = await verifyCrossDelegation(xdel, BOB_SA, ORG_AUDIENCE)
  ...
  try {
    ...
    const r2 = await verifyCrossDelegation(xdelStrict, BOB_SA, ORG_AUDIENCE)
    if ('error' in r2) {
...
test('issuance: DelegateBinding caveat encoding/decoding round-trips and is part of caveatsHash', async () => {
  ...
  const baseCaveats = [
    ...
    buildDataScopeCaveat(ORG_GRANTS),
