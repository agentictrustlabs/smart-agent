/**
 * Inbound service-auth middleware for family-mcp (Spec 007 Phase D).
 *
 * Mirrors the analogous middleware on hub-mcp, person-mcp, org-mcp, and
 * people-group-mcp. Family-mcp's surface today is OID4VCI credential
 * issuance + AnonCreds verification — no `/tools/*` routes yet — but the
 * MAC envelope shape is wired so a future tool surface (or signed
 * inter-service hop) can mount this middleware without reinventing the
 * verifier.
 *
 ...
 *
 * MAC key id is `a2a-to-family`. HMAC is symmetric — a2a-agent and
 * family-mcp share the secret (`A2A_INTERSERVICE_HMAC_KEY_FAMILY` in
 * local-aes mode, `AWS_KMS_MAC_KEY_ID_A2A_TO_FAMILY` in aws-kms mode, or
 * `GCP_KMS_MAC_A2A_TO_FAMILY_VERSION` in gcp-kms mode).
 *
 * NOTE: OID4VCI well-known endpoints + `/credential/*` + `/verify/*`
 * remain PUBLIC by protocol design. This middleware should only be
 * mounted on routes that represent system-internal authority (e.g.,
 * any future `/tools/*` surface).
 */
...
function inboundMacProvider(): KmsMacProvider {
  if (!cachedMacProvider) {
    cachedMacProvider = buildMcpMacProvider('family', process.env)
  }
...
export function requireInboundServiceAuth(
  ...
  return async (c: Context, next) => {
    ...
    try {
      ...
      return c.json(
        { error: `a2a-to-family key not configured: ${(err as Error).message}` },
        503,
      )
    }
