/**
 * Inbound service-auth middleware for geo-mcp (Spec 007 Phase D).
 *
 * Mirrors the analogous middleware on hub-mcp, person-mcp, org-mcp, and
 * people-group-mcp. Geo-mcp's surface today is OID4VCI credential
 ...
 *
 * MAC key id is `a2a-to-geo`. HMAC is symmetric — a2a-agent and geo-mcp
 * share the secret (`A2A_INTERSERVICE_HMAC_KEY_GEO` in local-aes mode,
 * `AWS_KMS_MAC_KEY_ID_A2A_TO_GEO` in aws-kms mode, or
 * `GCP_KMS_MAC_A2A_TO_GEO_VERSION` in gcp-kms mode).
 *
...
function inboundMacProvider(): KmsMacProvider {
  if (!cachedMacProvider) {
    cachedMacProvider = buildMcpMacProvider('geo', process.env)
  }
...
export function requireInboundServiceAuth(
  ...
  return async (c: Context, next) => {
    ...
    try {
      ...
      return c.json(
        { error: `a2a-to-geo key not configured: ${(err as Error).message}` },
        503,
      )
    }
