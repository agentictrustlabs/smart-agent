 ...
 * anything else. Uses OnChainResolver to fetch schema + credDef.
 */
...
export async function verifyCoachPresentation(
  resolver: OnChainResolver,
  presentationJson: string,
  presentationRequest: Record<string, unknown>,
): Promise<boolean> {
  const schema = await loadVerifiedSchema(resolver, MEMBERSHIP_SCHEMA_ID)
  const credDef = await loadVerifiedCredDef(resolver, MEMBERSHIP_CRED_DEF_ID)
