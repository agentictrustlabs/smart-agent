...
export function TreasuryClient({ targetAddress, targetName }: { targetAddress: string; targetName: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
      <code style={{
        background: '#fafafa', border: '1px solid #2a2a3e', borderRadius: 6,
        padding: '0.5rem 0.75rem', fontSize: '0.8rem', fontFamily: 'monospace',
        color: '#1a1a2e', flex: 1, minWidth: 300,
      }}>
        {targetAddress}
      </code>
      <button onClick={copyAddress} style={{ background: '#e0e0e0', color: '#1a1a2e', padding: '0.5rem 1rem', whiteSpace: 'nowrap' }}>
