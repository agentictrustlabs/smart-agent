...
export async function provisionHolderWalletAction(): Promise<{ success: boolean; holderWalletId?: string; error?: string }> {
  try {
    ...
    const built = await person.callTool<{ action: WalletAction & { expiresAt: string } }>(
      ...
      { principal, type: 'ProvisionHolderWallet', counterpartyId: 'self', purpose: 'initial provision' },
    )
