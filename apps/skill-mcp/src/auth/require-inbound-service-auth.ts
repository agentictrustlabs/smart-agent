/**
 * Inbound service-auth middleware for skill-mcp (Spec 007 Phase D).
 *
 * Mirrors the analogous middleware on hub-mcp, person-mcp, org-mcp, and
 * people-group-mcp. Skill-mcp's surface today is OID4VCI credential
 * issuance — no `/tools/*` routes yet — but the MAC envelope shape is
 * wired so a future tool surface (or signed inter-service hop) can mount
 * this middleware without reinventing the verifier.
 *
 ...
 *
 * MAC key id is `a2a-to-skill`. HMAC is symmetric — a2a-agent and
 * skill-mcp share the secret (`A2A_INTERSERVICE_HMAC_KEY_SKILL` in
 * local-aes mode, `AWS_KMS_MAC_KEY_ID_A2A_TO_SKILL` in aws-kms mode, or
 * `GCP_KMS_MAC_A2A_TO_SKILL_VERSION` in gcp-kms mode).
 *
 * NOTE: OID4VCI well-known endpoints + `/credential/*` remain PUBLIC by
 * protocol design. This middleware should only be mounted on routes that
 * represent system-internal authority (e.g., any future `/tools/*` surface).
 */
...
function inboundMacProvider(): KmsMacProvider {
  if (!cachedMacProvider) {
    cachedMacProvider = buildMcpMacProvider('skill', process.env)
  }
...
export function requireInboundServiceAuth(
  ...
  return async (c: Context, next) => {
    ...
    try {
      ...
      return c.json(
        { error: `a2a-to-skill key not configured: ${(err as Error).message}` },
        503,
      )
    }
